...
В ходе выполения работы, я ознакомился с тем, как взаимодействовать с Web API с помощью скрипта Python, осежил в памяти, как поднимать контейнеры, работать с API, писать код для этих задач. Считаю данную лабораторную работу полезной для собственного развития, как разработчика.:)
